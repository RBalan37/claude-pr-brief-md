...
allowed-tools: Bash, Read, Write, Grep, Glob, WebFetch, mcp__atlassian__getJiraIssue, mcp__atlassian__searchJiraIssuesUsingJql, mcp__linear__*, mcp__asana__*, mcp__notion__*, mcp__github__*
...
several bolded points, a data/control flow as a small diagram, Anticipated impact).
Inside each section, write direct, unambiguous prose — do not fragment one section
into a header per sub-point. Not every claim needs a `file:line`; cite code where it
genuinely grounds a claim, not as a formality. A flow is often clearer as a small
diagram (a fenced code block with arrows works fine, see the example) than as prose.
Don't pad the document to look thorough — reviewers are overwhelmed by length, not
impressed by it.
...
5. **Adapts to the unit of work.** A single feature/fix gets one narrative. An epic
   ...
   AMVP-158213" line). A broader initiative gets goal/process/impact framing instead.
...
```bash
git fetch origin
git checkout origin/master -- .   # or read in place if already on master/main
```
...
- **Check the story's assumptions against the code.** If the story assumes something
  that isn't true today (the way the AMS→RMS example found the data path already
  built and only the trigger missing), say so — that reframes the whole scope.
...
- **Data / control flow**: a small diagram (fenced code block, arrows) when a flow
  is involved — usually clearer than a prose walkthrough.
